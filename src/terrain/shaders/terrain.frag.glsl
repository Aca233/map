...
float getCountryBorder(vec2 uv) {
    ...
    vec2 aaOffset = max(dUV * 0.45, (1.0 / u_mapSize) * 0.2);

    float b0 = countryBorderCore(uv, sampleScale);
    float b1 = countryBorderCore(uv + vec2( aaOffset.x, 0.0), sampleScale);
    float b2 = countryBorderCore(uv + vec2(-aaOffset.x, 0.0), sampleScale);
    float b3 = countryBorderCore(uv + vec2(0.0,  aaOffset.y), sampleScale);
    float b4 = countryBorderCore(uv + vec2(0.0, -aaOffset.y), sampleScale);

    // 用 max 避免交界节点被平均稀释，并做轻微 gamma 平衡
    float border = max(max(max(b0, b1), max(b2, b3)), b4);
    border = pow(border, 0.86);
    float aa = max(fwidth(border), 0.012);
    return smoothstep(0.08 - aa, 0.80 + aa, border);
}
...
float getStateBorder(vec2 uv) {
    ...
    float b0 = stateBorderCore(uv, sampleScale);
    float b1 = stateBorderCore(uv + vec2( aaOffset.x, 0.0), sampleScale);
    float b2 = stateBorderCore(uv + vec2(-aaOffset.x, 0.0), sampleScale);
    float b3 = stateBorderCore(uv + vec2(0.0,  aaOffset.y), sampleScale);
    float b4 = stateBorderCore(uv + vec2(0.0, -aaOffset.y), sampleScale);

    // 用 max 代替均值，避免交界节点被平均后“冲淡”
    float border = max(max(max(b0, b1), max(b2, b3)), b4);
...
float getCountryBorderGlow(vec2 uv) {
    ...
    for (int dy = -2; dy <= 2; dy++) {
        for (int dx = -2; dx <= 2; dx++) {
            ...
            vec3 neighbor = texture2D(u_countryLUT, uv + vec2(float(dx), float(dy)) * texel).rgb;
            float d = smoothstep(0.002, 0.015, length(center - neighbor));
            diff += d * (1.0 - dist / 3.0);
        }
    }

    return clamp(diff / 6.0, 0.0, 1.0);
}
...
// 海洋渲染
// ============================================================
vec3 getOceanColor(float height, vec3 normal, vec3 viewDir) {
    // 海洋改为纯色（按需求去除波纹/深浅渐变/高光）
    return vec3(0.23, 0.45, 0.67);
}
...
vec3 getTerrainColor(float height, vec3 normal) {
    float seaLvl = u_seaLevel;
    float slope = 1.0 - normal.y; // 坡度：0=平坦，1=垂直

    // 地形颜色定义
    vec3 beach    = vec3(0.78, 0.72, 0.52);
    vec3 grass    = vec3(0.28, 0.52, 0.22);
    vec3 darkGrass = vec3(0.18, 0.42, 0.14);
    vec3 forest   = vec3(0.12, 0.36, 0.12);
    vec3 mountain = vec3(0.48, 0.43, 0.38);
    vec3 rock     = vec3(0.40, 0.38, 0.35);
    vec3 snow     = vec3(0.92, 0.92, 0.96);

    // 程序化噪声（打破色带）
    float n1 = fbm(v_worldPos.xz * 0.3, 3);
    float n2 = noise2D(v_worldPos.xz * 1.5);
    float n3 = noise2D(v_worldPos.xz * 5.0) * 0.15; // 微细节

    // 高度偏移（噪声扰动过渡区域）
    float h = height + (n1 - 0.5) * 0.04;

    vec3 color;
    if (h < seaLvl + 0.015) {
        // 沙滩
        float t = smoothstep(seaLvl, seaLvl + 0.015, h);
        color = beach + vec3(n2 * 0.05);
    } else if (h < 0.48) {
        // 草地
        float t = smoothstep(seaLvl + 0.015, 0.48, h);
        color = mix(grass, darkGrass, t + n1 * 0.3);
        color += vec3(n3 * 0.5, n3, n3 * 0.3); // 微变化
    } else if (h < 0.62) {
        // 草地到森林过渡
        float t = smoothstep(0.48, 0.62, h);
        vec3 grassMixed = mix(grass, darkGrass, n1);
        color = mix(grassMixed, forest, t + n2 * 0.15);
    } else if (h < 0.78) {
        // 森林到山地过渡
        float t = smoothstep(0.62, 0.78, h);
        color = mix(forest, mountain, t + n1 * 0.1);
    } else {
        // 山地到雪山
        float t = smoothstep(0.78, 0.95, h);
        color = mix(mountain, snow, t);
        // 雪线噪声：更自然的雪覆盖
        float snowNoise = fbm(v_worldPos.xz * 2.0, 3);
        color = mix(color, snow, smoothstep(0.4, 0.7, snowNoise) * t);
    }

    // 陡峭区域显示岩石
    if (slope > 0.3 && height > seaLvl + 0.02) {
        float rockMix = smoothstep(0.3, 0.7, slope);
        color = mix(color, rock + vec3(n2 * 0.08), rockMix * 0.7);
    }
...
float getRiverMask(vec2 uv) {
    ...
    float mask = center;
    // 保持细线前提下，让主干略有存在感
    mask = max(mask, majorRing * 0.12);
    mask = max(mask, majorDiag * 0.04);

    // 支流按中心像素可见性映射；主干使用扩展后 mask
    float aa = max(fwidth(mask), 0.001);
    float minorMask = smoothstep(0.30 - aa, 0.72 + aa, center);
    float majorMask = smoothstep(0.46 - aa, 0.97 + aa, mask);
    return mix(minorMask, majorMask, majorCenter);
}
...
vec3 applyRivers(vec3 baseColor, bool isSea, vec2 uv) {
    ...
    // HOI4 风格：偏青蓝，主干更深
    vec3 riverDeep = vec3(0.05, 0.27, 0.48);
    vec3 riverLight = vec3(0.17, 0.52, 0.75);
    vec3 riverColor = mix(riverDeep, riverLight, riverMask);

    // 提升可见性（不增加几何粗细）
    vec3 carved = baseColor * (1.0 - 0.14 * riverMask);
    vec3 mixed = mix(carved, riverColor, 0.52 * riverMask);

    // 轻微反光
    mixed += vec3(0.04, 0.08, 0.12) * (0.08 * riverMask);
    return mixed;
}
...
void main() {
    // 读取地块颜色
    vec3 provinceRgb = texture2D(u_provinceMap, v_uv).rgb;

    // 获取国家颜色
    vec3 countryColor = texture2D(u_countryLUT, v_uv).rgb;

    // 获取 State 颜色
    vec3 stateRgb = texture2D(u_stateLUT, v_uv).rgb;

    // 获取 Strategic Region 颜色
    vec3 strategicRegionRgb = texture2D(u_strategicRegionLUT, v_uv).rgb;
    ...
    if (u_mapMode == 0) {
        // === 政治模式 ===
        if (isSea) {
            baseColor = getOceanColor(v_height, v_normal, viewDir);
        } else {
            vec3 terrainColor = getTerrainColor(v_height, v_normal);
            float terrainLuma = dot(terrainColor, vec3(0.299, 0.587, 0.114));
            float slope = 1.0 - clamp(v_normal.y, 0.0, 1.0);

            // 先用地形明度调制国家色，避免“纯色直接贴在地形上”
            vec3 colorized = countryColor * mix(0.82, 1.22, terrainLuma);

            // 坡面/山脊压暗 + 朝光面提亮，强化立体层次
            float sunTerm = clamp(dot(normalize(v_normal), normalize(vec3(0.5, 1.0, 0.3))), 0.0, 1.0);
            float reliefShade = mix(0.84, 1.12, sunTerm);
            float slopeDarken = smoothstep(0.18, 0.75, slope);
            reliefShade *= (1.0 - slopeDarken * 0.12);

            // 高海拔增加轻微冷调，拉开山地层次
            float highland = smoothstep(0.60, 0.92, v_height);
            vec3 highlandTint = mix(vec3(1.0), vec3(0.90, 0.94, 1.05), highland * 0.25);

            vec3 politicalColor = colorized * reliefShade * highlandTint;

            // 保留部分地形色相，避免“颜色糊在模型表面”
            baseColor = mix(terrainColor, politicalColor, 0.66);

            // 保留低强度细节噪声，降低颗粒感
            float terrainDetail = (noise2D(v_worldPos.xz * 8.0) - 0.5) * 0.04;
        ...
        if (isSea) {
            ...
            baseColor = getTerrainColor(v_height, v_normal);
        }
    } else if (u_mapMode == 2) {
        // === 高度图模式 ===
        // 用伪彩色代替纯灰度
        ...
        if (isSea) {
            ...
            // 微妙的纹理保留
            float terrainDetail = noise2D(v_worldPos.xz * 8.0) * 0.04;
            baseColor += vec3(terrainDetail);
        }
    }

    // 海洋保持纯色，不叠加河流/光照/边界/雾效
    if (isSea) {
        gl_FragColor = vec4(getOceanColor(v_height, v_normal, viewDir), 1.0);
        return;
    }
    ...
    // ==================== Blinn-Phong 光照 ====================
    if (!isSea || (u_mapMode != 0 && u_mapMode != 3)) {
        // 陆地光照（海洋已在 getOceanColor 中处理了高光）
    ...
    // ==================== Strategic Region 级别悬停高亮 ====================
    if (u_hoveredStrategicRegionColor.r >= 0.0) {
    ...
    // ==================== Strategic Region 级别选中高亮 ====================
    if (u_selectedStrategicRegionColor.r >= 0.0) {
    ...
    if (u_mapMode == 0) {
        ...
        if (border > 0.0) {
            baseColor = mix(baseColor, vec3(0.0), 0.11 * border);
        }
    } else if (u_mapMode == 2) {
        // 高度图模式：细线但不发虚
        float border = getBorder(v_uv);
        if (border > 0.0) {
            baseColor = mix(baseColor, vec3(0.0), 0.13 * border);
        }
    } else {
        // 行政区模式边界线：国家 > State（粗实线）
        float countryBdr = getCountryBorder(v_uv);
        float countryGlow = getCountryBorderGlow(v_uv);
        float stateBdr = getStateBorder(v_uv);
